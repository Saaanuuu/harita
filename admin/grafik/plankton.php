...
$sql = "SELECT tahun, jumlah_taksa_fito, kelimpahan_fito, jumlah_taksa_zoo, kelimpahan_zoo FROM t_detail_titik_pengamatan WHERE t_titik_pengamatan_no = $_GET[titikPengamatanNo] ORDER BY tahun";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        array_push($dataPoints, array(
            "tahun" => $row["tahun"],
            "jumlah_taksa_fito" => $row["jumlah_taksa_fito"],
            "kelimpahan_fito" => $row["kelimpahan_fito"],
            "jumlah_taksa_zoo" => $row["jumlah_taksa_zoo"],
            "kelimpahan_zoo" => $row["kelimpahan_zoo"]
        ));
    }
}
...
<script>
    ...
    var parameters = ['jumlah_taksa_fito', 'kelimpahan_fito', 'jumlah_taksa_zoo', 'kelimpahan_zoo']; // Kolom yang baru
