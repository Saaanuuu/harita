...
$sql = "SELECT tahun, jumlah_spesies_ikan, kelimpahan_ikan FROM t_detail_titik_pengamatan WHERE t_titik_pengamatan_no = $_GET[titikPengamatanNo] ORDER BY tahun";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        array_push($dataPoints, array(
            "tahun" => $row["tahun"],
            "jumlah_spesies_ikan" => $row["jumlah_spesies_ikan"],
            "kelimpahan_ikan" => $row["kelimpahan_ikan"]
        ));
    }
}
...
<script>
    ...
    var parameters = ['jumlah_spesies_ikan', 'kelimpahan_ikan']; // Kolom yang baru
