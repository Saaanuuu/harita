...
$sql = "SELECT tahun, jumlah_taksa_bn, kepadatan_bn FROM t_detail_titik_pengamatan WHERE t_titik_pengamatan_no = $_GET[titikPengamatanNo] ORDER BY tahun";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        array_push($dataPoints, array(
            "tahun" => $row["tahun"],
            "jumlah_taksa_bn" => $row["jumlah_taksa_bn"],
            "kepadatan_bn" => $row["kepadatan_bn"]
        ));
    }
}
...
<script>
    ...
    var parameters = ['jumlah_taksa_bn', 'kepadatan_bn']; // Kolom yang baru
